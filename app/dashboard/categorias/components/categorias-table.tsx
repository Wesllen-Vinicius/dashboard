...
export function CategoriasTable({
  ...
  const renderSubComponent = useCallback(({ row }: { row: Row<Categoria> }) => {
    const categoria = row.original;
    const details = [
      { label: "ID do Registro", value: categoria.id, className: "col-span-1 sm:col-span-2" },
      {
        label: "Data/Hora de Criação",
        value:
          categoria.createdAt && categoria.createdAt instanceof Timestamp
            ? format(
                categoria.createdAt.toDate(),
  ...
  const columns: ColumnDef<Categoria>[] = useMemo(
    () => [
      {
        id: "expander",
        header: () => null,
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Badge
            variant={row.original.status === "ativo" ? "success" : "destructive"}
          >
