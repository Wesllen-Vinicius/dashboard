...
import { IconPencil, IconFileInvoice, IconLoader, IconFileDownload, IconFileX, IconAlertTriangle, IconX } from "@tabler/icons-react";
import { GenericTable } from "@/components/generic-table";
import { Venda } from "@/lib/schemas";

type VendaComDetalhes = Venda & { clienteNome?: string };

const NfeActionButton = ({
    venda,
    onPrepareNFe,
}: {
    ...
    switch (status) {
        case "autorizado":
            return (
                <div className="flex gap-2">
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button
                                    ...
                                    asChild
                                >
                                    <a href={venda.nfe?.url_danfe} target="_blank" rel="noopener noreferrer">
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button
                                    ...
                                    asChild
                                >
                                    <a href={venda.nfe?.url_xml} target="_blank" rel="noopener noreferrer">
                                        <IconFileX className="h-4 w-4" />
                                    </a>
                                </Button>
                            </TooltipTrigger>
                            <TooltipContent>
                    ...
                    {/* Exemplo de botão de cancelamento de NF-e */}
                    {/* <TooltipProvider>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button size="sm" variant="destructive" onClick={() => onCancelarNFe(venda)}>
        ...
        case "erro_autorizacao":
            return (
                <TooltipProvider>
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Button size="sm" variant="destructive" onClick={() => onPrepareNFe(venda)}>
                                <IconAlertTriangle className="h-4 w-4 mr-2" />
                                Erro
                            </Button>
                        </TooltipTrigger>
                        <TooltipContent>
                            <p>Houve um erro. Clique para corrigir e tentar novamente.</p>
                        </TooltipContent>
                    </Tooltip>
                </TooltipProvider>
            );
        ...
        default:
            return (
                <Button size="sm" onClick={() => onPrepareNFe(venda)}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                {row.original.produtos.map((p, i) => (
                    <div key={i} className="text-xs p-2.5 border rounded-lg bg-background shadow-sm">
                        ...
                        <p>
                            <strong>Preço Un.:</strong> R$ {p.precoUnitario.toFixed(2)}
                        </p>
                    </div>
                ))}
            </div>
...
export function VendasTable({ vendas, onEdit, onPrepareNFe, isReadOnly }: VendasTableProps) {
    const columns: ColumnDef<VendaComDetalhes>[] = [
        ...
        {
            header: "Valor Final",
            cell: ({ row }) => `R$ ${(row.original.valorFinal || row.original.valorTotal).toFixed(2)}`,
        },
        {
            header: "Pagamento",
            cell: ({ row }) => (
                <Badge variant={row.original.status === "Paga" ? "success" : "warning"}>
                    {row.original.status}
                </Badge>
            ),
        },
        {
            id: "nfe_status_action",
            header: "NF-e",
            cell: ({ row }) => (
                <NfeActionButton venda={row.original} onPrepareNFe={onPrepareNFe} />
            ),
        },
        {
            id: "edit_action",
                    ...
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Button
                                ...
                                disabled={isReadOnly || row.original.nfe?.status === "autorizado"}
                            >
