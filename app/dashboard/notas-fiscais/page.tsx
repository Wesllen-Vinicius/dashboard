...
import {
  ...
  cancelarNFe,
  emitirNFe,
} from "@/lib/services/nfe.services";
...
import { syncNotasFiscais } from "@/lib/services/sync-nfe.services";
import { syncNotaFiscalPorRef } from "@/lib/utils/nfe/syncPorRef";
...
const getStatusInfo = (
  ...
  switch (status) {
    case "autorizado":
      return { text: "Autorizada", variant: "success" };
    case "processando_autorizacao":
      return { text: "Processando", variant: "warning" };
    case "cancelado":
      return { text: "Cancelada", variant: "secondary" };
    case "erro_autorizacao":
      return { text: "Erro", variant: "destructive" };
    case "erro_cancelamento":
      return { text: "Erro ao Cancelar", variant: "destructive" };
    default:
      return { text: status || "Não Emitida", variant: "outline" };
  }
};
...
const handleSyncNotas = async () => {
  setIsSyncing(true);
  const toastId = toast.loading("Sincronizando notas fiscais...");
  try {
    await syncNotasFiscais(); // 🔁 sincroniza vendas com Focus NFe
    toast.success("Notas fiscais sincronizadas com sucesso!", { id: toastId });
  } catch (error: any) {
    toast.error("Erro ao sincronizar notas fiscais", {
      id: toastId,
  ...
  const vendasComNFe = useMemo(() => {
    return vendas
      ...
      .map((venda) => ({
        ...venda,
        clienteNome:
          clientes.find((c) => c.id === venda.clienteId)?.nomeRazaoSocial ||
          "N/A",
      }));
  }, [vendas, clientes]);
  ...
  const filteredVendas = useMemo(() => {
    return vendasComNFe.filter((v) => {
      const statusMatch =
        statusFilter === "todos" || v.nfe?.status === statusFilter;
      const dateMatch =
        dateRange?.from && dateRange.to
          ? new Date(v.data) >= dateRange.from &&
            new Date(v.data) <= dateRange.to
          : true;
  ...
  const statusCounts = useMemo(
    () => ({
      processando_autorizacao: vendasComNFe.filter(
        (v) => v.nfe?.status === "processando_autorizacao"
      ).length,
      autorizado: vendasComNFe.filter(
        (v) => v.nfe?.status === "autorizado"
      ).length,
      erro_autorizacao: vendasComNFe.filter(
        (v) => v.nfe?.status === "erro_autorizacao"
      ).length,
      cancelado: vendasComNFe.filter(
        (v) => v.nfe?.status === "cancelado"
      ).length,
    }),
    [vendasComNFe]
  );
  ...
  const handlePrepareNFe = async (venda: VendaComNFe) => {
    ...
    try {
      ...
      if (!cliente || !empresaInfo) {
        throw new Error("Dados do cliente ou da empresa não encontrados.");
      }
      setNfePreviewData({
        venda,
        empresa: empresaInfo,
  ...
  const handleConfirmAndEmitNFe = async (formData: {
    venda: Venda;
    empresa: CompanyInfo;
    cliente: Cliente;
    ...
    try {
      const resultado = await emitirNFe(
        ...
        formData.empresa,
        formData.cliente,
        formData.todosProdutos,
        formData.todasUnidades
      );
      const nfeUpdateData = {
  id: resultado.ref,
  ...
  url_xml: resultado.caminho_xml_nota_fiscal
      ...
      if (resultado.status === "autorizado") {
        ...
        toast.info("NF-e em processamento. Consulte o status em breve.", {
          id: toastId,
        });
      } else {
        const errorMessage = resultado.erros
          ? resultado.erros[0].mensagem
          : resultado.mensagem_sefaz || "Erro desconhecido";
        toast.error(`Falha na emissão: ${errorMessage}`, {
          id: toastId,
          duration: 10000,
        });
      }
  ...
  const handleConsultarStatus = async (venda: VendaComNFe) => {
    ...
    try {
      ...
      const nfeUpdateData = {
        id: resultado.ref ?? venda.nfe.id,
        status: resultado.status ?? "erro_desconhecido",
        url_danfe: resultado.url_danfe ?? venda.nfe.url_danfe,
        url_xml: resultado.url_xml ?? venda.nfe.url_xml,
      };
  ...
  const handleCancelarNFe = async () => {
    ...
    try {
      const resultado = await cancelarNFe(
        selectedVenda.nfe.id,
        cancelJustificativa
      );

      if (resultado.status === "cancelado") {
        await updateVenda(selectedVenda.id!, {
          nfe: { ...selectedVenda.nfe, status: "cancelado" },
        });
        ...
        const errorMessage = resultado.erros
          ? resultado.erros[0].mensagem
          : resultado.mensagem_sefaz || "Falha no cancelamento";
  ...
  const columns: ColumnDef<VendaComNFe>[] = [
    ...
    { header: "Cliente", accessorKey: "clienteNome" },
    {
      header: "Valor",
      cell: ({ row }) =>
        `R$ ${(row.original.valorFinal || row.original.valorTotal).toFixed(2)}`,
    },
    {
      header: "Status NF-e",
          ...
          <div className="flex justify-end gap-2">
            {status === "erro_autorizacao" && (
              <Button
                variant="destructive"
                size="sm"
                onClick={() => handlePrepareNFe(venda)}
              >
                <IconRefresh className="h-4 w-4 mr-1" /> Re-emitir
              </Button>
            )}
            {status === "processando_autorizacao" && (
              <Button
                variant="secondary"
                size="sm"
                onClick={() => handleConsultarStatus(venda)}
              >
            ...
            {status === "autorizado" && (
              <>
                <Button variant="outline" size="sm" asChild>
                  <a
                    href={venda.nfe?.url_danfe}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    <IconFileDownload className="h-4 w-4 mr-1" /> DANFE
                  </a>
                </Button>
                <Button
                  variant="destructive"
    ...
    <div className="container mx-auto py-8 px-4 md:px-6 space-y-6">
      ...
      <Dialog open={isCanceling} onOpenChange={setIsCanceling}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Forneça uma justificativa com no mínimo 15 caracteres.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <StatusCard
          ...
          count={statusCounts.processando_autorizacao}
          icon={IconClock}
          variant="warning"
          onClick={() => setStatusFilter("processando_autorizacao")}
        />
        <StatusCard
          title="Com Erro na Emissão"
          count={statusCounts.erro_autorizacao}
          icon={IconAlertTriangle}
          variant="destructive"
          onClick={() => setStatusFilter("erro_autorizacao")}
        />
        <StatusCard
          title="Canceladas"
  ...
  <Button onClick={handleSyncNotas} disabled={isSyncing}>
    {isSyncing && <IconLoader className="mr-2 h-4 w-4 animate-spin" />}
    {isSyncing ? "Sincronizando..." : "Sincronizar Notas"}
  </Button>
</div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Consulte, baixe e cancele as notas fiscais emitidas no sistema.
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  variant="outline"
  onClick={() =>
    syncNotaFiscalPorRef("ztrHWTPV3wnGdxMpoLdT").then(() =>
      toast.success("NF-e sincronizada com sucesso!")
    )
  }
>
  Sincronizar NF-e por Ref
</Button>
